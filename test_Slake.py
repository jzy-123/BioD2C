...
import transformers
from Datasets.Slake import Slake_Dataset
...
class DataArguments:
    ...
    Test_data_path: str = field(default='./Data/Slake1.0/test_close.csv', metadata={"help": "Path to the training data."})
...
def main():
    ...
    Test_dataset_close = Slake_Dataset(data_args.Test_data_path, data_args.tokenizer_path,img_dir=data_args.img_dir,mode='Test',start=row_count)
    ...
    Test_dataset_open = Slake_Dataset(data_args.Test_data_path.replace('close.csv','open.csv'), data_args.tokenizer_path,img_dir=data_args.img_dir,mode='Test',text_type='blank',start=row_count)
    ...
    with open('./rescsv/Slake/SLAKE_close_'+model_args.ckp.split('/')[-3]+'_'+ model_args.ckp.split('/')[-2]+'.csv', mode='w') as outfile:
    ...
    with open('./rescsv/Slake/SLAKE_open_'+model_args.ckp.split('/')[-3]+'_'+ model_args.ckp.split('/')[-2]+'.csv', mode='w') as outfile:
            writer = csv.writer(outfile)
            writer.writerow(['Figure_path','Question','Pred','Label'])
